...
/* Unit test checks proper number of links for Home page. */
test("Home page links check", () => {
  const wrapper = shallow(<Home />);
  expect(wrapper.find("Link").length).toEqual(3);
});

/* Unit test checks proper number of images in Home page. */
test("Home page images check", () => {
  const wrapper = shallow(<Home />);
  expect(wrapper.find("img").length).toEqual(4);
});

/* Unit test checks proper number of images in Home page. */
test("Home page Marquee check", () => {
  const wrapper = shallow(<Home />);
...
/* Simple Unit Tests for headers in About Page */
test("About page paragraph tags check", () => {
  const wrapper = shallow(<About />);
...
/*Unit test checks number of sort buttons in Books Page */
test("checks number of sort buttons in books page", () => {
  const wrapper = shallow(<Books />);
  expect(wrapper.find("SortButton").length).toEqual(5);
});

/*Unit test checks number of filter buttons in Books Page */
test("checks number of filter buttons in books page", () => {
  const wrapper = shallow(<Books />);
  expect(wrapper.find("FilterButton").length).toEqual(4);
});
...
test("checks number of FontAwesomeIcons in books page", () => {
  const wrapper = shallow(<Books />);
  expect(wrapper.find("FontAwesomeIcon").length).toEqual(1);
});

/*Unit test checks number of input tags in Books Page */
test("checks number of input tags in books page", () => {
  const wrapper = shallow(<Books />);
  expect(wrapper.find("input").length).toEqual(1);
});

/*Unit test checks number of card headers in Books Page */
test("checks number of input headers in books page", () => {
  const wrapper = shallow(<Books />);
...
test("Make sure that Book Item component exists given data test 1", () => {
  const book1 = {
    ...
    id: 1,
    image:
      "http://books.google.com/books/content?id=jcJWAAAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
    language: "en",
...
/* Grid Item test for Books*/
test("Make sure that Book Item component contain Card Flip given data test 1", () => {
  const book1 = {
    author_names: ["James Dashner"],
    avg_rating: 4.0,
    description:
  ...
  expect(instance.find("ReactCardFlip").length).toEqual(1);
});

/* Grid Item  test for Books*/
test("Make sure that Book Item component exists given data test 2", () => {
  const book2 = {
    author_names: ["William T. Vollmann"],
    avg_rating: 4.0,
    description:
    ...
    id: 5,
    image:
      "http://books.google.com/books/content?id=Pu0kDwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
    language: "en",
    maturity_rating: "NOT_MATURE",
    name: "Europe Central",
    num_ratings: 9,
...
/* Grid Item  test for Books*/
test("Make sure that Book Item component has CardFlip given data test 2", () => {
  const book2 = {
    author_names: ["William T. Vollmann"],
    avg_rating: 4.0,
    description:
...
test("Make sure that Book Item component exists given data test 3", () => {
  const book3 = {
    ...
    id: 55,
    image:
      "http://books.google.com/books/content?id=SuRdVgJR9F8C&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
    language: "en",
    maturity_rating: "NOT_MATURE",
    name: "Old Glory",
    num_ratings: null,
    ...
    price: 11.99,
    purchase_link:
      "https://play.google.com/store/books/details?id=SuRdVgJR9F8C&rdid=book-SuRdVgJR9F8C&rdot=1&source=gbs_api",
    year: "2011-05-25",
  };
  const instance = mount(
    <Router>
...
/* Grid Item  test for Books*/
test("Make sure that Book Item component has CardFlip given data test 3", () => {
  const book3 = {
    author_names: ["Jonathan Raban"],
    avg_rating: null,
    description:
...
/* Grid Item test for Books*/
test("Make sure that Book Item component exists given data test 4", () => {
  const book4 = {
    author_names: ["Jonathan Raban"],
    avg_rating: null,
    description:
...
/* Grid Item test for Books*/
test("Make sure that Book Item component contain CardFlip given data test 4", () => {
  const book4 = {
    author_names: ["Jonathan Raban"],
    avg_rating: null,
    description:
